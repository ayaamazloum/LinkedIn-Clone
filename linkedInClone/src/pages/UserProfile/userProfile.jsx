import './style.css';
import Header from '../Header/header';
import axios from 'axios';
import { useState } from 'react';

const UserProfile = () => {
    const [followed, setFollowed] = useState("");

    const userInfo = JSON.parse(localStorage.getItem('userProfileId'));

    return <div className="user-profile-page full-width pb-70">
        <Header></Header>
        <div className='info-container flex column white-bg mt-30 more-rounded primary-shadow'>
            <div className='cover-image'></div>
            <img className='result-profile-img' src="data:image/png;base64,"/>
            <h2>{userInfo.role=="user"? userInfo.first_name + " " + userInfo.last_name: userInfo.company_name}</h2>
            <p className='grey-text'>{userInfo.role}</p>
            <p className='grey-text'>{userInfo.location}</p>
            <p>{userInfo.bio}</p>
            <button
                className='white-bg primary-text extra-rounded mb-10'
                onClick={async () => {
                    const formData = new FormData();
                    formData.append('follower_id', JSON.parse(localStorage.getItem('user'))['user_id']);
                    formData.append('followed_id', JSON.parse(localStorage.getItem('userProfileId'))['user_id']);
                    console.log(JSON.parse(localStorage.getItem('userProfileId'))['user_id']);
                    const response = await axios.post("http://127.0.0.1/LinkedIn-Clone/Backend/follow.php", formData);
                    setFollowed("following");
                }}
            >{followed=="following" ? 'Following' : 'Follow'}</button>
        </div>
    </div>
};

export default UserProfile;